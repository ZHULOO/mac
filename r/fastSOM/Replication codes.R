...
for (i in 1:546) {
  dasam=da[i:(i+199),]
  consam=control[i:(i+199),]
  lagda=data.frame(VARselect(dasam[,-1],season = 52,exogen = consam))
  estsam=VAR(dasam[,-1],p=lagda[1,1],exogen = consam)
  sigsam=summary(estsam)$covres
  A=MAcoefs(sigsam,Acoef(estsam),2)
  ressam=data.frame(soi_avg_exact(sigsam,A,ncores = 2))
  ressam=ressam[1,(1:3)]
  SOIdata=rbind(SOIdata,ressam)
}
