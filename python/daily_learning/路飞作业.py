...
province = []
for add in address:
    province.append(add[1][0:2])
province = list(set(province))      # province = ['山东', '内蒙', '青海', '辽宁'] 
pro_list = [[i] for i in province]  
pro_list1 = copy.deepcopy(pro_list) # pro_list1 = [['山东'], ['内蒙'], ['青海'], ['辽宁']] 

for item in address:
    str1 = item[1][0:2]             # str1 = '北京'
    str1 = str1.split()             # 字符串用列表[]括起来,str1 = ['北京']
    ind1 = pro_list1.index(str1)    # ['北京']在pro_list1中的位置
    pro_list[ind1].append(item)     # 将一条信息附加到pro_list中相同位置上的列表中

dict1 = {}
dict2 = {}
for i in pro_list:
    dict2 = {i[0]:i[1:]}            # 将每一条信息中的两部分内容分裂为键值对
    dict1.update(dict2)             # 每一个键值对添加到同一个字典中
print(dict1)
